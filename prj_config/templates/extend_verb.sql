{# Compute variables for add_process_descriptor#}
 ...
 {% set config_process_id = _keynames.full_keyname("PD",old_pd['CONFIG_PROCESS_ID']) %}
 {% set old_process_desc ="Verb is currently point to this process descriptor:\n"+old_pd  | string %}

{% if old_pd['PD_TYPE'] != 'NULL'%}
 {% set process_descriptor_type = _keynames.keyname("PDT",old_pd['PD_TYPE']) %}
{% else %}
...
{% endif %}
 ...
 {% set repository_path=          __repository_path | description("repository_path") | print(old_process_desc)
					            | codepath()
