...
{% set __label = label  | default("null")%}
...
--Adding field {{__entity_field_keyname}} to view {{__entity_view_keyname}}
...
{# If the layout group does not exist then create it #}
...
{%endif%}
{#nested ifs doesnt seem to work #}
{% if __group_row is none and __group_name != 'NULL' %}
INSERT INTO LOCALISED_FIELD (OBJECT_TYPE,OBJECT_INSTANCE,OBJECT_VERSION,FIELD_NAME,LOCALE,LOOKUP_LOCALE,TEXT,LONG_TEXT,LAST_MODIFIED,IS_DELETED) VALUES (
	...
	'N');
{% endif %}
{# Note that some scripts add LOCALISED_FIELD with field name 'views.<<ViewNAME>>.fields.<<fieldName>>.label' however this doesn not seem to be required #}
{# Compute keyname #}
...
{% else %}
	...
	{% set pd_help_message = "Typical process descriptors used in km content entities are:\nFieldEditorTagSetSelection: for tags when creating/editing content\n TagSetSelectionViewerComponent: for tags when viewing content\nFieldEditorRichText: for body fields like public/private when editing content\nFieldViewerRichText: for body fields like public/private when viewing content" %}	
	{% set __show_as_pd = show_as_pd | suggest(field_pds) | print(pd_help_message)%}
